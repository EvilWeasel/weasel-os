...
- Date: 2026-08-24
- Change: Kept Vesktop's AppImage desktop entry/icons in the package and initially routed Niri ScreenCast/Screenshot requests through `xdg-desktop-portal-wlr` with linear capture buffers.
- Pitfall/Root cause: `appimageTools.wrapType2` exported only the executable until the upstream desktop artifacts were installed explicitly. The xdp-wlr workaround was invalid for this Niri session: its chooser returned no valid output and logged `selection cancelled` / `no output found`, removing the portal UI entirely. Niri officially supports `xdg-desktop-portal-gnome`; preserve that backend and diagnose the remaining hybrid-GPU stream handoff separately.
- Verification: `bash tests/test-vesktop-wayland-integration.sh`; Niri's documented screencasting requirements; live D-Bus introspection proving `org.gnome.Mutter.ScreenCast` and the GNOME portal ScreenCast interface are present; remote laptop evaluation/build/flake checks and a login-free portal restart.
