...
### 2026-08-24 (Vesktop launcher and hybrid-GPU screen sharing)
- Date: 2026-08-24
- Change: Kept Vesktop's AppImage desktop entry/icons in the package and routed only Niri ScreenCast/Screenshot requests through `xdg-desktop-portal-wlr` with linear capture buffers; GNOME/GTK remain the defaults for other portals.
- Pitfall/Root cause: `appimageTools.wrapType2` exported only the executable until the upstream desktop artifacts were installed explicitly. On the hybrid laptop, the GNOME portal negotiated an NVIDIA DMA-BUF modifier while Electron consumed on Intel, so the chooser completed without delivering a stream.
- Verification: `bash tests/test-vesktop-wayland-integration.sh`; `nix-instantiate --parse profiles/system/laptop.nix`; `nix-instantiate --parse packages/vesktop-bin.nix`; `nix eval --no-write-lock-file .#nixosConfigurations.nixy-laptop.config.system.build.toplevel.drvPath`; full `nix build --no-link --no-write-lock-file -L .#nixosConfigurations.nixy-laptop.config.system.build.toplevel`; built Vesktop desktop-entry/icon assertions via `VESKTOP_PACKAGE`; evaluated Niri portal and wlr settings; `nix flake check --no-write-lock-file -L`.
